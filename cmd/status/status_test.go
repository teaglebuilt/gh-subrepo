...
func TestStatusCmd_Integration(t *testing.T) {
	...
	assert.NoError(t, ExecCmd(rootDir, "git", "init"))

	remoteRepo, err := os.MkdirTemp("", "remote-subrepo-*")
	assert.NoError(t, err)
	defer os.RemoveAll(remoteRepo)

	assert.NoError(t, ExecCmd(remoteRepo, "git", "init", "--bare"))

	tmpWork, err := os.MkdirTemp("", "remote-workspace-*")
	assert.NoError(t, err)
	defer os.RemoveAll(tmpWork)

	assert.NoError(t, ExecCmd("", "git", "clone", remoteRepo, tmpWork))

	os.WriteFile(filepath.Join(tmpWork, "README.md"), []byte("Initial remote content"), 0644)
	assert.NoError(t, ExecCmd(tmpWork, "git", "add", "."))
	assert.NoError(t, ExecCmd(tmpWork, "git", "commit", "-m", "Initial commit"))
	assert.NoError(t, ExecCmd(tmpWork, "git", "push", "origin", "master"))

	subrepoDir := "vendor/lib"
	assert.NoError(t, ExecCmd(rootDir, "git", "clone", remoteRepo, filepath.Join(rootDir, subrepoDir)))
	...
	assert.NoError(t, ExecCmd(rootDir, "git", "add", "."))
	assert.NoError(t, ExecCmd(rootDir, "git", "commit", "-m", "Add subrepo"))
	...
	assert.NoError(t, ExecCmd(tmpWork, "git", "add", "."))
	assert.NoError(t, ExecCmd(tmpWork, "git", "commit", "-m", "Add CHANGELOG"))
	assert.NoError(t, ExecCmd(tmpWork, "git", "push", "origin", "master"))
